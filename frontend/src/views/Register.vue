...
export default {
  emits: ['authenticated'],
  data(){ return { name:'', email:'', password:'', error:null } },
  methods: {
    submit(){
      this.error = null
      fetch('/api/register', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ name:this.name, email:this.email, password:this.password }) })
        .then(r=>r.json())
        .then(data=>{
